# manage_users_window_ui.py
from PyQt5.QtWidgets import (
    QDialog, QVBoxLayout, QHBoxLayout, QTableWidget, QTableWidgetItem,
    QPushButton, QMessageBox
)
from PyQt5.QtCore import Qt
from user_edit_dialog import UserEditDialog  # نافذة إضافة/تعديل المستخدم

class ManageUsersDialog(QDialog):
    """
    نافذة لإدارة المستخدمين: عرض، إضافة، تعديل وحذف مع دعم الصلاحيات.
    يحمي حساب المبرمج (saif103) من التعديل أو الحذف من الواجهة.
    """
    ...
    def setup_ui(self):
        ...
        self.table.setHorizontalHeaderLabels(
            ["Username", "Role", "Can Add", "Can Edit", "Can Delete", "Last Login"]
        )
    ...
    def load_users(self):
        ...
        try:
            ...
            for row_index, user in enumerate(users):
                ...
                self.table.setItem(row_index, 0, QTableWidgetItem(user.get("username", "")))
                self.table.setItem(row_index, 1, QTableWidgetItem(user.get("role", "")))
    ...
    def add_user(self):
        """فتح نافذة لإضافة مستخدم جديد بعد التحقق من الصلاحيات."""
        ...
        if dialog.exec_():
            ...
            # حماية من إنشاء حساب باسم المبرمج
            if data["username"] == "saif103":
                QMessageBox.warning(self, "Access Denied", "Cannot create developer account from UI.")
                return
            try:
    ...
    def edit_user(self):
        """فتح نافذة لتعديل مستخدم محدد بعد التحقق من الصلاحيات."""
        if self.current_role not in ("admin", "developer", "owner"):
        ...
        # حماية حساب المبرمج من التعديل
        if username == "saif103":
            QMessageBox.warning(self, "Access Denied", "Developer account cannot be edited from UI.")
            return
    ...
    def delete_user(self):
        """حذف المستخدم المحدد بعد التأكيد والتحقق من الصلاحيات."""
        if self.current_role not in ("admin", "developer", "owner"):
        ...
        username = self.table.item(row, 0).text()
        # حماية حساب المبرمج من الحذف
        if username == "saif103":
            QMessageBox.warning(self, "Access Denied", "Developer account cannot be deleted from UI.")
            return
